<!DOCTYPE html>
<html lang="es">
    <head>
        <title>Grey Dot : Agencia de Publicidad</title>
        <meta charset="UTF-8"/>
        <meta name="description" content="Agencia de Publicidad" />
        <link rel="icon" href="img/favicon.ico">
        <link rel="stylesheet" href="css/estilo.css"/>
        <link rel="stylesheet" href="css/vendor/jquery.fancybox.css"/>
        <link rel="stylesheet" href="css/vendor/jquery.fancybox-thumbs.css"/>
        <link rel="stylesheet" href="css/vendor/nanoscroller.css">
    </head>
    <body>
        <nav>
            <ul>
                <li class="nav_arrow"><a href="#"><span class="icon-arrow-left"></a></span></li>
                <li id="nav_cierre_izq"></li>
                <li><a href="#home">Home</a></li>
                <li><a href="#como_pensamos">Cómo pensamos</a></li>
                <li><a href="#clientes">Clientes</a></li>
                <li><a href="#nuestro_trabajo">Nuestro Trabajo</a></li>
                <li><a href="#fotografia">Fotografía</a></li>
                <li><a href="#stand">Stand</a></li>
                <li><a href="#contacto">Contacto</a></li>
                <li id="nav_cierre_der"></li>
                <li class="nav_arrow"><a href="#"><span class="icon-uniE601"></a></span></li>
            </ul>
        </nav>

        <!--/****************HOME***************/-->
        <section id="home">
            <h2>Home</h2>
        </section>


        <!--/***********COMO PENSAMOS***********/-->
        <section id="como_pensamos">
            <h2>Cómo pensamos</h2>
        </section>

        <!--/*************CLIENTES**************/-->
        <section id="clientes">
            <h2>Clientes</h2>
            <article id="clientes_cuadro">
                <div id="toggle_clientes" href="#"></div>
                <div id="contenedor_clientes">
                    <div id="separador"></div>
                    <div id="clientes_lista" class="nano">
                        <ul class="nano-content">
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                            <li>ELEMENTO</li>
                        </ul>
                    </div>
                </div>
            </article>
        </section>

        <!--/**********NUESTRO TRABAJO**********/-->
        <section id="nuestro_trabajo">
            <h2>Nuestro <span>trabajo</span></h2>
            <table class="tabla_galeria">
                <tr>
                  <td>Print</td>
                  <td class="boton_mas"><div id="nuestro_trabajo_print" href="javascript:;"></div></td>
                </tr>
                <tr>
                  <td>TV</td>
                  <td class="boton_mas"><div id="nuestro_trabajo_tv" href="javascript:;"></div></td>
                </tr>
                <tr>
                  <td>Radio</td>
                  <td class="boton_mas"><div id="nuestro_trabajo_radio" href="javascript:;"></div></td>
                </tr>
            </table>
        </section>

        <!--/**********FOTOGRAFÍA**********/-->
        <section id="fotografia">
            <h2>Fotografía</h2>
            <p>
                ‘He scents thy footsteps in the snow
                Wheresoever thou dost go,
                Thro’ the wintry hail and rain.
                When wilt thou return again?

            </p>
        </section>

        <!--/**********STAND**********/-->
        <section id="stand">
            <h2>Stand</h2>
            <p>
                ‘He scents thy footsteps in the snow
                Wheresoever thou dost go,
                Thro’ the wintry hail and rain.
                When wilt thou return again?
            </p>
        </section>

        <!--/**********CONTACTO**********/-->
        <section id="contacto">
            <h2>Contacto</h2>
            <p>
                ‘He scents thy footsteps in the snow
                Wheresoever thou dost go,
                Thro’ the wintry hail and rain.
                When wilt thou return again?
            </p>

        <!--/**JAVASCRIPT**/-->
        <script src="http://ajax.googleapis.com/ajax/libs/jquery/1/jquery.min.js"></script>
        <script src="js/vendor/jquery.easing.1.3.js"></script>
        <script src="js/vendor/jquery.stellar.js"></script>
        <script src="js/vendor/jquery.nanoscroller.js"></script>
        <script src="js/vendor/jquery.fancybox.js"></script>
        <script src="js/vendor/jquery.fancybox-thumbs.js"></script>
        <script src="js/vendor/jquery.nanoscroller.js"></script>
        <script src="js/js.js"></script>
    </body>
</html>